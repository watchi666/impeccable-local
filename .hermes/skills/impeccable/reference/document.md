...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file and ask the user directly to clarify. whether to refresh, overwrite, or merge.
...
4. **Refresh the session cache.** Run `node .hermes/skills/impeccable/scripts/load-context.mjs` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
3. Run `node .hermes/skills/impeccable/scripts/load-context.mjs` once so the seed lands in conversation for the rest of the session.
