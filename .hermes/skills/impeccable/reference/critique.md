...
CLI scan:
```bash
node .hermes/skills/impeccable/scripts/detect.mjs --json [--fast] [target]
```
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `SOUL.md` contains a `## Design Context` section from `impeccable teach`, also generate 1-2 project-specific personas from the audience/brand info.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .hermes/skills/impeccable/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .hermes/skills/impeccable/scripts/critique-storage.mjs trend <slug> 5
   ```
...
**After presenting findings**, use targeted questions based on what was actually found. ask the user directly to clarify. These answers will shape the action plan.
