...
```bash
node .hermes/skills/impeccable/scripts/load-context.mjs
```
...
- **Both exist**: ask the user directly to clarify. Ask which file to refresh. Skip the one the user doesn't want changed.
...
ask the user directly to clarify. Ask only about what you couldn't infer from the codebase.
...
If the signal is genuinely split (e.g. a product with a big marketing landing), ask the user directly to clarify. Ask which register describes the **primary** surface. The register can be overridden per task later, but PRODUCT.md carries one default.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node .hermes/skills/impeccable/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.
...
Optionally ask the user directly to clarify. Ask whether they'd like a brief summary of PRODUCT.md appended to SOUL.md for easier agent reference. If yes, append a short **Design Context** pointer section there.
